...
export function ViolationTabBar({ activeTab, onTabChange }: Props) {
  return (
    <div className="p-4 bg-gray-50/50 border-b border-gray-100 flex justify-center overflow-x-auto touch-pan-x">
      <div className="inline-flex bg-gray-200/70 p-1.5 rounded-xl shadow-inner gap-2 w-full md:w-auto min-w-max">
        <Button
          variant="ghost"
          onClick={() => onTabChange("entry")}
          className={`flex-1 md:flex-none relative flex items-center justify-center gap-2 px-4 sm:px-8 py-3 rounded-lg text-sm font-bold transition-all duration-300 whitespace-nowrap ${
            activeTab === "entry"
